...
async function assets(env,request){const s=await authCsrf(env,request,'admin');if(request.method==='GET'){const courseId=new URL(request.url).searchParams.get('courseId');const rows=courseId?await env.DB.prepare('SELECT * FROM assets WHERE course_id=? OR course_id IS NULL ORDER BY created_at DESC LIMIT 500').bind(courseId).all():await env.DB.prepare('SELECT * FROM assets ORDER BY created_at DESC LIMIT 500').all();return json({assets:rows.results||[]});}
  const url=new URL(request.url),filename=cleanName(url.searchParams.get('filename')||'upload.bin'),courseId=url.searchParams.get('courseId')||null,kind=url.searchParams.get('kind')||'general';const size=Number(request.headers.get('Content-Length')||0);if(size>MAX_ASSET_BYTES)return json({error:'File exceeds the 100 MB Cloudflare Free request limit'},413);if(!request.body)return json({error:'Upload body is empty'},400);const id=randomId(),key=`uploads/${new Date().toISOString().slice(0,10)}/${id}-${filename}`,mime=mimeFor(filename,request.headers.get('Content-Type')||'');await env.CONTENT.put(key,request.body,{httpMetadata:{contentType:mime}});await env.DB.prepare('INSERT INTO assets(id,course_id,storage_key,filename,mime_type,size_bytes,kind,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?)').bind(id,courseId,key,filename,mime,size,kind,s.id,now()).run();await audit(env,s.id,'asset.uploaded','asset',id,{filename,kind,size,courseId});return json({ok:true,asset:{id,courseId,filename,mimeType:mime,sizeBytes:size,kind}},201);
}
...
async function serveAsset(env,request,id){await requireUser(env,request);const asset=await env.DB.prepare('SELECT * FROM assets WHERE id=?').bind(id).first();if(!asset)return new Response('Not found',{status:404});const obj=await env.CONTENT.get(asset.storage_key);if(!obj)return new Response('Not found',{status:404});const headers=new Headers();obj.writeHttpMetadata(headers);headers.set('Cache-Control','private,max-age=3600');return new Response(obj.body,{headers});}
...
async function scormContent(env,request,packageId,pathParts){const s=await requireUser(env,request);const pkg=await env.DB.prepare('SELECT * FROM scorm_packages WHERE id=?').bind(packageId).first();if(!pkg)return new Response('Not found',{status:404});if(pkg.course_id&&!(await env.DB.prepare("SELECT id FROM enrollments WHERE user_id=? AND course_id=? AND status IN ('active','completed')").bind(s.id,pkg.course_id).first())&&s.role!=='admin')return new Response('Forbidden',{status:403});const path=pathParts.map(decodeURIComponent).join('/');if(path.includes('..'))return new Response('Bad path',{status:400});const obj=await env.CONTENT.get(`${pkg.storage_prefix}/${path}`);if(!obj)return new Response('Not found',{status:404});const headers=new Headers();obj.writeHttpMetadata(headers);headers.set('Cache-Control','private,max-age=600');return new Response(obj.body,{headers});}
