...
async function register(env,request){
  if(!env.BREVO_API_KEY||!env.MAIL_FROM_EMAIL)return json({error:'Learner registration is temporarily unavailable while email verification is being configured.'},503);
...
async function listCourses(env,request,admin=false){await authCsrf(env,request,admin?'admin':null);const q=new URL(request.url).searchParams.get('q')?.trim()||'';const p=`%${q}%`;const sql=admin?`SELECT c.*,COUNT(DISTINCT CASE WHEN e.status='active' THEN e.user_id END) learners,COUNT(DISTINCT m.id) modules,COUNT(DISTINCT l.id) lessons FROM courses c LEFT JOIN enrollments e ON e.course_id=c.id LEFT JOIN modules m ON m.course_id=c.id LEFT JOIN lessons l ON l.module_id=m.id WHERE c.title LIKE ? OR c.category LIKE ? GROUP BY c.id ORDER BY c.updated_at DESC`:`SELECT * FROM courses WHERE status='published' AND (title LIKE ? OR category LIKE ?) ORDER BY updated_at DESC`;const rows=await env.DB.prepare(sql).bind(p,p).all();return json({courses:rows.results||[]});}
async function saveCourse(env,request,id=null){const s=await authCsrf(env,request,'admin');const b=await bodyJson(request);const title=String(b.title||'').trim();if(!title)return json({error:'Course title is required'},400);const slug=await uniqueSlug(env,b.slug||title,id);if(b.status==='published'){if(!id)return json({error:'Create the course as Draft first, add published content, then publish it.'},409);const ready=await env.DB.prepare('SELECT COUNT(*) n FROM lessons l JOIN modules m ON m.id=l.module_id WHERE m.course_id=?').bind(id).first();if(Number(ready?.n||0)===0)return json({error:'Add at least one published content item before publishing this course'},409);}const estimatedMinutes=Math.max(1,Math.min(100000,Number(b.estimatedMinutes||30)));const values=[title,String(b.shortDescription||''),String(b.description||''),String(b.category||'Professional Skills'),['Foundation','Intermediate','Expert'].includes(b.level)?b.level:'Foundation',['draft','published','archived'].includes(b.status)?b.status:'draft',b.coverAssetId||null,Math.max(0,Number(b.xpReward||200)),b.certificateEnabled===false?0:1,Math.min(100,Math.max(0,Number(b.passingScore||70))),estimatedMinutes,now()];
  if(id){await env.DB.prepare(`UPDATE courses SET slug=?,title=?,short_description=?,description=?,category=?,level=?,status=?,cover_asset_id=?,xp_reward=?,certificate_enabled=?,passing_score=?,estimated_minutes=?,updated_at=? WHERE id=?`).bind(slug,...values,id).run();await audit(env,s.id,'course.updated','course',id,{title});return json({ok:true,id});}
  const cid=randomId();await env.DB.prepare(`INSERT INTO courses(id,slug,title,short_description,description,category,level,status,cover_asset_id,xp_reward,certificate_enabled,passing_score,estimated_minutes,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(cid,slug,...values,now()).run();await audit(env,s.id,'course.created','course',cid,{title});return json({ok:true,id:cid},201);
}
...
async function assets(env,request){
  ...
  if(request.method==='GET'){
    ...
    const rows=courseId?await env.DB.prepare('SELECT a.*,(SELECT COUNT(DISTINCT m.course_id) FROM lessons l JOIN modules m ON m.id=l.module_id WHERE l.asset_id=a.id) usage_courses FROM assets a WHERE a.course_id=? OR a.course_id IS NULL ORDER BY a.created_at DESC LIMIT 500').bind(courseId).all():await env.DB.prepare('SELECT a.*,(SELECT COUNT(DISTINCT m.course_id) FROM lessons l JOIN modules m ON m.id=l.module_id WHERE l.asset_id=a.id) usage_courses FROM assets a ORDER BY a.created_at DESC LIMIT 500').all();
...
async function scormUpload(env,request){
  ...
  let moduleId=null,lessonId=null;
  if(courseId){
    let target=requestedModuleId&&await env.DB.prepare('SELECT id FROM modules WHERE id=? AND course_id=?').bind(requestedModuleId,courseId).first();
    if(!target){
      const mid=randomId(),position=Number((await env.DB.prepare('SELECT COALESCE(MAX(position),0)+1 p FROM modules WHERE course_id=?').bind(courseId).first())?.p||1);
      await env.DB.prepare('INSERT INTO modules(id,course_id,title,description,position,xp_reward,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)').bind(mid,courseId,'Course Content','Published learning content',position,40,now(),now()).run();
      target={id:mid};
    }
    moduleId=target.id;
    const position=Number((await env.DB.prepare('SELECT COALESCE(MAX(position),0)+1 p FROM lessons WHERE module_id=?').bind(moduleId).first())?.p||1);
    lessonId=randomId();
...
async function courseDetailBySlug(env,request,slug){const s=await requireUser(env,request);const c=s.role==='admin'?await env.DB.prepare('SELECT id FROM courses WHERE slug=?').bind(slug).first():await env.DB.prepare('SELECT id FROM courses WHERE slug=? AND status=\'published\'').bind(slug).first();if(!c)return json({error:'Course not found'},404);return courseDetail(env,request,c.id,s.role==='admin');}
